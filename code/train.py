...
from untils import generate_random
import torch.nn as nn
import torch.nn.functional as F
...
def train(config, model, train_iter, dev_iter, test_iter,writer):
    ...
    scheduler = torch.optim.lr_scheduler.StepLR(optimizer,2, gamma=0.8, last_epoch=-1)#每2个epoch学习率衰减为原来的一半
    ...
    for epoch in range(config.num_epochs):
        ...
        acc_list=[]
        hidden = model.init_hidden(2, config.batch_size)
        print('Epoch [{}/{}]'.format(epoch + 1, config.num_epochs))
        for i, (trains, labels) in enumerate(train_iter):

            labels=labels.view(-1)
            outputs,hidden= model(trains,hidden)
        ...
        ge = generate_random(model, config, '孤')
        print('长度:{}生成:{}'.format(len(ge), ge))
...
def test(config, model, test_iter):
    ...
    msg = 'Test Loss: {0:>5.2},  Test Acc0: {1:>6.2%}'
...
if __name__ == '__main__':
    ...
    train_iter=DataLoader(train_data, batch_size=config.batch_size,shuffle=True,drop_last=True)   ##训练迭代器
    dev_iter = DataLoader(dev_data, batch_size=config.batch_size,shuffle=True,drop_last=True)      ###验证迭代器
    test_iter = DataLoader(test_data, batch_size=config.batch_size,shuffle=True,drop_last=True)   ###测试迭代器
    # 训练
    mynet=Mybert(config)
    ## 模型放入到GPU中去
...
#tensorboard --logdir=log/lstm/1677916143.9477277 --port=6006